...
if __name__ == '__main__':
    ...
    torch.save(model.state_dict(), 'mobilenetv2_aircraft_classifier.pth')

    # To perform inference, load the model and set it to evaluation mode
    model = models.mobilenet_v2()
    model.classifier[1] = nn.Linear(model.last_channel, num_classes)
    model.load_state_dict(torch.load('mobilenetv2_aircraft_classifier.pth'))
    model = model.to(device)
    model.eval()
    ...
    # Assuming you have an image at 'path_to_image.jpg'
    ...
    # Assuming 'frame' is the current video frame and 'detections' is a list of bounding boxes from YOLOv8
    # Each detection is a tuple: (x1, y1, x2, y2, confidence)
    # detections = [...]  # Replace with your detection results
    # frame = ...  # Replace with your video frame

    # Uncomment and replace placeholders when integrating with YOLOv8
    # for detection in detections:
    #     x1, y1, x2, y2, conf = detection
    #     # Ensure coordinates are integers
    #     x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
    #     # Crop the detected aircraft
    #     cropped_img = frame[y1:y2, x1:x2]
    #     # Convert OpenCV image (BGR) to PIL image (RGB)
    #     pil_img = Image.fromarray(cv2.cvtColor(cropped_img, cv2.COLOR_BGR2RGB))
    #     # Predict aircraft type
    #     predicted_class = predict_image(pil_img)
    #     # Annotate image
    #     cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
    #     cv2.putText(frame, predicted_class, (x1, y1 - 10),
    #                 cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)

    # Display the frame (if using a video stream)
    # cv2.imshow('Aircraft Detection and Classification', frame)
    # cv2.waitKey(0)
